...
def parse():
    ...
    parse.add_argument('filename', type=str, default="dataset3")
...
def data_enc(filename):
    ...
    try:
        ...
        logger.error("Please check that the data set is located at ./datasets/filename.csv")
...
def format_data(filename="dataset3", test_size=None, train_size=None, random_state=None, stratify=None):
    try:
        ...
        logger.error("Please encode the desired dataset using the command python routers/data_encoder.py filename")
